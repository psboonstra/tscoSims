...
# the LRT.
#
# The procedure evaluated here is, precisely: CONSTRAINED-ML CPPO WITH THE
# 2-DF LIKELIHOOD-RATIO TEST OF b1 = b2 = 0 AND chi-square_2 CALIBRATION. It is
# a defined comparator, not "CPPO as practiced". Peterson and Harrell (1990,
# Section 3.2) test the association of a constrained variable with the response
# by a TWO-DF WALD TEST of beta = 0, gamma = 0 -- the test most exposed to a
# boundary MLE, since it needs the information matrix there. Their Section 3.1
# score tests are for the proportional-odds hypothesis (gamma = 0), not for
# association. Standard VGAM usage does not enforce the parameter space, and the
# constrained fitter below is this study's own. (Their "constrained" is the
# linear constraint on the PPO deviation that defines CPPO; the inequality
...
##
## PARAMETERIZATION (rewritten 2026-09-18, after external review)
## ---------------------------------------------------------------
...
## that. The supremum then has no finite optimizer, BFGS "converges" wherever
...
# there; an OBSERVED category driven to zero is penalized through the floor,
# and the optimizer moves away from it.
...
# Chain rule from (d nll / d alpha_j) to the spacing parameterization:
...
# KKT check at a returned optimum, using the analytic gradient (verified
# against finite differences in verification/r_cppo_fitter_check.R). The
# check is on the PER-OBSERVATION gradient, gradient / n, against a fixed
# tolerance: achieved residuals are ~1e-7 at n = 200 and ~2e-7 at n = 1000, so
# 1e-6 has an order of magnitude of headroom.
#
# A gradient threshold is not scale-invariant in the predictors. This one is
# calibrated to the design here -- A binary, W standard normal -- and would
# need revisiting for predictors on a very different scale.
.cppo_kkt <- function(gr, v, lower, n, tol = 1e-6, ...) {
  g <- gr(v, ...) / n
...
# The objective floors observed-cell probabilities at 1e-300 so that a stray
# step cannot produce log(0); that makes it differ from the exact multinomial
# log-likelihood wherever an OBSERVED cell is driven below the floor. The
# optimum must never be there. This returns the smallest probability the fit
# assigns to any observed cell; a converged fit must clear `min_obs_prob_tol`
# by a wide margin or it is not accepted as exact constrained ML.
.cppo_min_obs_prob <- function(pr, Yidx) {
  min(pr[cbind(seq_along(Yidx), Yidx)])
}
min_obs_prob_tol <- 1e-12
...
po_direct_fit <- function(X, Yidx, K) {
  ...
  q <- plogis(outer(-drop(X %*% beta), alpha, "+"))
  min_obs_prob <- .cppo_min_obs_prob(cppo_direct_probs_from_q(q), Yidx)
  ...
  list(
    ...
    converged = best$convergence == 0L && kkt$ok && min_obs_prob > min_obs_prob_tol,
...
# nll there EQUALS the reduced nll, so any improvement the optimizer finds
...
cppo_direct_fit <- function(X, Yidx, a_col, G, K, red = NULL) {
  ...
  # Reduced solution lifted into the full parameterization: beta_A = 0, u = s_M.
  red_par <- red$par
  beta_full <- numeric(p)
  ...
  beta <- best$par[(M + 1):(M + p)]
  gamma <- u - best$par[M]
  lin <- outer(-drop(X %*% beta), alpha, "+") - outer(gamma * X[, a_col], G)
  min_obs_prob <- .cppo_min_obs_prob(cppo_direct_probs_from_q(plogis(lin)), Yidx)
...
## ---------------------------------------------------------------------------
## Rao score test of H0: b1 = b2 = 0 (added 2026-09-18)
##
## WHY IT IS HERE. The LRT above compares the constrained full-model MLE with
## the reduced-model MLE, and in a sparse cell the full-model MLE sits on the
## boundary of the CPPO parameter space (64% of cppo_alt draws at n = 200),
## where the chi-square_2 reference is not justified. The score test never
## computes the full-model MLE. It is evaluated at the REDUCED fit -- PO in W,
## which has no A parameters, is always interior, and is regular -- so its
## chi-square_2 null law needs nothing about the alternative's parameter space.
## Under H0 the truth is interior to the CPPO space (b2 = 0 > -s_M whenever
## every level is observed in the population), so the standard Rao argument
## goes through. The boundary problem does not arise, by construction.
##
## Peterson and Harrell (1990, Section 3.1) use Rao score tests, evaluated
## under the proportional-odds fit, for the PROPORTIONAL-ODDS hypothesis
...
## is empty the score is just minus a small expected count. That is an honest
## statement of how much information the data carry about b2, not a
## calibration failure.
##
## PARAMETERIZATION. The natural one, theta = (alpha_1..alpha_M, beta_W, b1,
## b2), with lin_ij = alpha_j - beta_W W_i - A_i (b1 + b2 G_j). No constraint
## transform is needed because nothing is fitted here: the null estimate is
## taken from po_direct_fit() and (b1, b2) are set to zero. The score S is the
## gradient of the log-likelihood at that point; the nuisance components are
...
## not zero in the nuisance directions; the test's premise fails and it is
## returned as NA with a tag (0.3% of null draws, 4% of cppo_alt at n = 200).
## ---------------------------------------------------------------------------

# Per-row gradient of the NEGATIVE log-likelihood in the natural
# parameterization: an n x p matrix whose column sums are the gradient.
.cppo_natural_grad_rows <- function(theta, X, Yidx, a_col, w_cols, G, M) {
  alpha <- theta[1:M]
  beta_w <- theta[M + seq_along(w_cols)]
  b1 <- theta[M + length(w_cols) + 1L]
  b2 <- theta[M + length(w_cols) + 2L]
  A <- X[, a_col]
  eta <- drop(X[, w_cols, drop = FALSE] %*% beta_w)
  lin <- outer(-eta, alpha, "+") - outer(A, b1 + b2 * G)
  q <- plogis(lin)
  D <- .cppo_direct_D(q, Yidx)              # d nll_i / d lin_ij
  rs <- rowSums(D)
  cbind(D,
        -X[, w_cols, drop = FALSE] * rs,
        -A * rs,
        -A * drop(D %*% G))
}

.cppo_natural_grad <- function(theta, X, Yidx, a_col, w_cols, G, M) {
  colSums(.cppo_natural_grad_rows(theta, X, Yidx, a_col, w_cols, G, M))
}

# Category probabilities at theta, n x K.
.cppo_natural_probs <- function(theta, X, a_col, w_cols, G, M) {
  alpha <- theta[1:M]
  beta_w <- theta[M + seq_along(w_cols)]
  b1 <- theta[M + length(w_cols) + 1L]
  b2 <- theta[M + length(w_cols) + 2L]
...
cppo_score_test <- function(X, Yidx, a_col, G, K, red,
                            w_cols = setdiff(seq_len(ncol(X)), a_col),
                            info = c("expected", "observed")) {
  info <- match.arg(info)
  M <- K - 1L
  n <- length(Yidx)
  theta0 <- c(red$alpha, red$beta, 0, 0)
  p <- length(theta0)

  # Score = -(gradient of nll).
  S <- -.cppo_natural_grad(theta0, X, Yidx, a_col, w_cols, G, M)

  if (info == "expected") {
    # I = sum_i E_theta0[ g_i g_i' ] = sum_i sum_k p_ik g_ik g_ik', where g_ik is
    # row i's score if its outcome were k. The sign of g does not matter.
    pr <- .cppo_natural_probs(theta0, X, a_col, w_cols, G, M)
    H <- matrix(0, p, p)
    for (k in seq_len(K)) {
      gk <- .cppo_natural_grad_rows(theta0, X, rep(k, n), a_col, w_cols, G, M)
      H <- H + crossprod(gk * sqrt(pmax(pr[, k], 0)))
    }
  } else {
    # Observed information: central-difference Hessian of the analytic
    # gradient, symmetrized. Kept for the verification script's contrast; NOT
    # used by the simulation -- see the note above.
    h <- 1e-5 * pmax(1, abs(theta0))
    H <- matrix(NA_real_, p, p)
    for (k in seq_len(p)) {
      tp <- theta0; tp[k] <- tp[k] + h[k]
      tm <- theta0; tm[k] <- tm[k] - h[k]
      H[, k] <- (.cppo_natural_grad(tp, X, Yidx, a_col, w_cols, G, M) -
                 .cppo_natural_grad(tm, X, Yidx, a_col, w_cols, G, M)) / (2 * h[k])
    }
    H <- (H + t(H)) / 2
  }
...
fxn_cppo <- function(dat, test_dat, levels_y,
  ...
  if (any(lvl_counts == 0L)) {
    ...
    # s_M and u active) and the statistic reduces numerically to the PO
    # statistic. That identity does NOT by itself establish the unconditional
    # law of the resulting adaptive statistic -- the support event is
    # data-dependent -- so the tag says what is known and no more. Recorded,
    # not corrected: see the policy note above.
    if (lvl_counts[K - 1L] == 0L) {
      warnings <- c(warnings,
        "cppo: departing level unobserved; deviation parameter unidentified, statistic reduces to PO's; chi-square_2 calibration not justified")
    }
    use_direct <- TRUE
  }

  # The score test is computed on every path from a direct reduced fit, which
  # is cheap (~10 ms) and identical to the reduced model the LRT uses on the
  # direct path. It is returned alongside the LRT, never in place of it.
  Yidx_all <- match(as.character(dat$Y), levels_y)
  X_all <- cbind(A = dat$A, W = dat$W)
  score <- NULL
  score_tag <- "cppo: score test unavailable (non-standard G or unmatched levels)"
  if (last_cutpoint_G && !any(is.na(Yidx_all))) {
    red0 <- tryCatch(po_direct_fit(X_all[, "W", drop = FALSE], Yidx_all, K = K),
                     error = function(e) NULL)
    if (is.null(red0) || !red0$converged) {
      score_tag <- "cppo: score test unavailable (reduced fit failed)"
    } else if (red0$on_boundary) {
      # A level unobserved in both arms: the null fit has a spacing at zero,
      # the nuisance score is not zero, and the test is not regular.
      score_tag <- "cppo: score test not regular (reduced fit on the boundary: outcome level unobserved)"
    } else {
      score <- tryCatch(cppo_score_test(X_all, Yidx_all, a_col = 1L, G = G, K = K, red = red0),
                        error = function(e) NULL)
      if (is.null(score) || !isTRUE(score$info_ok)) {
        score <- NULL
        score_tag <- "cppo: score test unavailable (information matrix not invertible)"
      }
    }
  }
  ...
  if (!use_direct) {
    ...
    if (fit_full$ok) {
      ...
      # VGAM cannot land exactly on the boundary, so `boundary` here is the
      # thresholded version (eps = 1e-3 in cppo_boundary_tags); the direct path
      # below reads it off the active constraints. `slack_min` is recorded so
      # the processor can apply any threshold it likes to both engines alike.
      boundary <- length(cppo_boundary_tags(slack)) > 0L
      slack_min <- suppressWarnings(min(slack, na.rm = TRUE))
      if (!is.finite(slack_min)) slack_min <- NA_real_
    }
