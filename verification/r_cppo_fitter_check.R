...
## ---------------------------------------------------------------------------
## 0. ANALYTIC GRADIENTS AGAINST FINITE DIFFERENCES -- a hard assertion.
##
## The optimizer and the KKT check both use the analytic gradient, so they
## cannot vouch for each other; only a finite-difference comparison is
## independent. Random interior points, points with one or both bounds active,
## several datasets, both models. Fails loudly rather than reporting.
## ---------------------------------------------------------------------------
cat("=== 0. analytic vs numerical gradient ===\n")
grad_worst <- 0
for (seed in 1:5) {
  set.seed(seed)
  dat0 <- make_covariates(n)
  dat0$Y <- draw_ordinal(true_prob_scenario(dat0, scenario), levels_y = levels_y)
  Y0 <- match(as.character(dat0$Y), levels_y)
  X0 <- cbind(A = dat0$A, W = dat0$W)
  lower_f <- c(-Inf, rep(0, M - 1L), rep(-Inf, 2L), 0)
  lower_r <- c(-Inf, rep(0, M - 1L), -Inf)

  test_points <- list(
    interior1 = c(rnorm(1), runif(M - 1L, 0.05, 1), rnorm(2, 0, 0.5), runif(1, 0.05, 1)),
    interior2 = c(rnorm(1), runif(M - 1L, 0.05, 1), rnorm(2, 0, 0.5), runif(1, 0.05, 1)),
    u_active  = c(rnorm(1), runif(M - 1L, 0.05, 1), rnorm(2, 0, 0.5), 0),
    sM_active = c(rnorm(1), runif(M - 2L, 0.05, 1), 0, rnorm(2, 0, 0.5), runif(1, 0.05, 1)),
    both      = c(rnorm(1), runif(M - 2L, 0.05, 1), 0, rnorm(2, 0, 0.5), 0)
  )
  for (nm in names(test_points)) {
    v <- test_points[[nm]]
    # With s_M = 0 two cutpoints coincide and category M has probability
    # EXACTLY zero for the rows whose arm has no deviation opening it up. An
    # OBSERVED row in such a cell puts the objective on its 1e-300 floor, where
    # it is not the log-likelihood and its gradient is not the analytic one
    # -- and a converged fit is never there (min_obs_prob is checked). So the
    # test is run on the rows for which the point is inside the smooth region.
    keep <- rep(TRUE, length(Y0))
    if (nm == "sM_active") keep <- !(Y0 == M & X0[, "A"] == 0)
    if (nm == "both")      keep <- Y0 != M
    Xk <- X0[keep, , drop = FALSE]; Yk <- Y0[keep]
    ga <- cppo_direct_grad(v, Xk, Yk, 1L, G, M)
    gn <- .cppo_num_grad(cppo_direct_nll, v, lower_f, X = Xk, Yidx = Yk, a_col = 1L, G = G, M = M)
    grad_worst <- max(grad_worst, max(abs(ga - gn) / pmax(1, abs(gn))))
    # Reduced model at the matching point (drop A's beta and u). The reduced
    # model has no deviation, so s_M = 0 zeroes category M for EVERY row.
    keep_r <- if (nm %in% c("sM_active", "both")) Y0 != M else rep(TRUE, length(Y0))
    vr <- c(v[1:M], v[M + 2L])
    Xr <- X0[keep_r, "W", drop = FALSE]; Yr <- Y0[keep_r]
    ga <- po_direct_grad(vr, Xr, Yr, M)
    gn <- .cppo_num_grad(po_direct_nll, vr, lower_r, X = Xr, Yidx = Yr, M = M)
    grad_worst <- max(grad_worst, max(abs(ga - gn) / pmax(1, abs(gn))))
  }
}
# One-sided differences at an active bound carry O(h) error, h = 1e-6; 1e-4
# relative is two orders of magnitude of headroom over that.
cat(sprintf("  worst relative discrepancy over 5 datasets x 5 points x 2 models: %.2e\n", grad_worst))
stopifnot("analytic gradient disagrees with finite differences" = grad_worst < 1e-4)
...
for (i in 1:nsim) {
  ...
  # KKT re-checked with the NUMERICAL gradient, so the verdict does not depend
  # on the same code the optimizer used.
  lower_f <- c(-Inf, rep(0, M - 1L), rep(-Inf, 2L), 0)
  kkt_num <- .cppo_kkt(function(v, ...) .cppo_num_grad(cppo_direct_nll, v, lower_f, ...),
                       full_new$par, lower_f, n = n,
                       X = X, Yidx = Yidx, a_col = 1L, G = G, M = M)
  ...
  out <- bind_rows(out, tibble(
    ...
    kkt_new = full_new$kkt_max_violation, kkt_numeric = kkt_num$max_violation,
    min_obs_prob = full_new$min_obs_prob, code_new = full_new$optim_code,
...
cat(sprintf("  KKT max violation (per-obs gradient): analytic median %.2e, max %.2e;  numerical max %.2e\n",
    median(out$kkt_new), max(out$kkt_new), max(out$kkt_numeric)))
cat(sprintf("  min observed-cell probability at the optimum, over draws: %.2e (floor 1e-300, tol 1e-12)\n",
    min(out$min_obs_prob)))
